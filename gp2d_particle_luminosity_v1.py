...
def particle_luminosity(emissfilename, particle):
	...
	#print scidata.shape
	#surface = 2*math.pi*(2*R+xdel)*xdel*zdel + 2*xdel*zdel + R*2*math.pi*zdel + (R+xdel)*2*math.pi*zdel
	#surface = math.pi*((R+xdel/2.)**2.-(R-xdel/2.)**2.)*zdel
	...
	#luminosity = np.sum(scidata, axis=(0,1))*surface
	#luminosity = np.sum(luminosity)*log_efactor*4.*math.pi/c*1.e6*ev2ergs
