...
###
MAP_FREQ = 10.e3
ACTUAL_FREQ = MAP_FREQ/(h/ev2ergs)
...
### UNITS
gp1*= FACTOR/ev2ergs*ACTUAL_FREQ/1.e6
...
# data_file = '/mnt/c/Users/psyko/Physics/proj_gal/m82_data/3.fits'
...
loglow = -4.
loghigh = 2.
V = np.logspace(loglow, loghigh, 20)
...
ax.text(0.05,0.1,'Max={:1.2e}'.format(np.amax(gp1)), ha='left', va='top', color='w', transform=ax.transAxes, bbox=dict(facecolor='black', edgecolor='none', alpha=0.5, pad=2))
ax.text(0.95,0.1,r'10 KeV', ha='right', va='top', color='w', transform=ax.transAxes, bbox=dict(facecolor='black', edgecolor='none', alpha=0.5, pad=2))
if MODE == 0:
	ax.text(0.05,0.9,'Model A', ha='left', va='bottom', color='w', transform=ax.transAxes, bbox=dict(facecolor='black', edgecolor='none', alpha=0.5, pad=2))
	plot_name = 'm82_gamma10KeV_from_rad_map_A'
elif MODE == 1:
	ax.text(0.05,0.9,'Model B', ha='left', va='bottom', color='w', transform=ax.transAxes)
	plot_name = 'm82_gamma10KeV_from_rad_map_B'
elif MODE == 2:
	ax.text(0.05,0.9,"Model B'", ha='left', va='bottom', color='w', transform=ax.transAxes)
	plot_name = 'm82_gamma10KeV_from_rad_map_Bp'
