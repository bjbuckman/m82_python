...
### INTERPOLATING
###
MAP_FREQ0 = 1.364e9 #GHz
MAP_FREQ1 = 0.326e9 #GHz
...
# for i in range(0,xnum):
	# for j in range(0,ynum):
		# d_pix = np.sqrt((x_flat-X[i][j])**2/beamx**2+(y_flat-Y[i][j])**2/beamy**2)
		# d_pix[d_pix<=1.]=1
		# d_pix[d_pix>1.]=0
		# NEW0[i][j] = np.sum(gp0_flat*d_pix)/np.sum(d_pix)
		# NEW1[i][j] = np.sum(gp1_flat*d_pix)/np.sum(d_pix)
...
beam = Ellipse(xy=[80,-80] , width=43.1, height=39.6)
ax.add_artist(beam)
beam.set_facecolor('white')
beam.set_alpha(0.6)

plot_name = 'm82_spectral_index_map_22-92_'
