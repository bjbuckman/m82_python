...
for n in range(0,num_graph):
	...
	for m in range(0,len(Z)):
		...
		# energy, e_tot = particle.electron_spectrum_total(dir_outfits+galdef_name[n]+'/'+'nuclei_full_54_'+galdef_name[n]+'.gz')
		
		energy, e_cor = particle.electron_spectrum_kinetic(dir_outfits+galdef_name[n]+'/'+'nuclei_full_54_'+galdef_name[n]+'.gz', 0., Z[m])
		
		e_species = e_cor.shape[0]
		
		# e_tot*= energy**exponent
		e_cor*= energy**exponent
		
		LINE = ['--','-.','--',':',':']
		ax.loglog(energy, np.sum(e_cor, axis=0)*FACTOR[n], label=r''+LABEL[m], c=color, linestyle='-', linewidth=LWIDTH, alpha=0.975)
		for i in range(0,e_species):
			ax.loglog(energy, e_cor[i]*FACTOR[n], label=None, c=color, linestyle=LINE[i], linewidth=LWIDTH, alpha=0.75)
		
ax.set_ylabel(r'$E \, \frac{dU}{dE}$ [MeV$^2$ cm$^{-3}$ MeV$^{-1}$]')
ax.set_xlabel(r'$E$ [MeV]')

# ax.text(1.e5,1.e-10,r'CRe$-$Model B', backgroundcolor='white')

ax.set_xlim([1.e1,1.0001e8])
# ax.set_ylim([2.e-8, 5.e-2])
ax.set_ylim([2.e-12, 1.e-5])

if MODEL == 0:
	ax.text(0.666,0.333,r'CRe$-$Model A', ha='center', va='center', color='k', transform=ax.transAxes, backgroundcolor='white')
	plot_name = 'plot_particle_e_A'
elif MODEL == 1:
	ax.text(0.666,0.333,r'CRe$-$Model B', ha='center', va='center', color='k', transform=ax.transAxes, backgroundcolor='white')
	plot_name = 'plot_particle_e_B'
elif MODEL == 2:
	ax.text(0.666,0.333,r"CRe$-$Model B'", ha='center', va='center', color='k', transform=ax.transAxes, backgroundcolor='white')
	plot_name = 'plot_particle_e_Bp'

# ax.legend(loc='upper right', prop={'size':15})

plt.tight_layout()
# plot_name = 'plot_particle_e_'
